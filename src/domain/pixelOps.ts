...
import type { BinaryMaskSurface, BrushShape, SwatchRef, PixelLayer, PixelSurface, PixelValue, Point } from "./types";
  ...
  for (const mirroredPoint of mirroredPoints(
    ...
    for (let yy = 0; yy < options.size; yy += 1) {
      for (let xx = 0; xx < options.size; xx += 1) {
        ...
        changed = setPixel(layer, x, y, options.swatchRef) || changed;
      }
    }
  }
...
export function floodFill(
  ...
  if (target === swatchRef) return false;
  ...
  while (stack.length > 0) {
    ...
    changed = setSurfacePixel(layer.surface, current.x, current.y, swatchRef) || changed;
    stack.push(
